...
pub struct SparsePattern<I: SparseIndex = u32> {
...
#[derive(Clone, Debug, PartialEq, Serialize, Deserialize)]
pub struct SparseMatrix<T, I: SparseIndex = u32> {
    pub pattern: Arc<SparsePattern<I>>,
    pub values: Vec<T>,
}
impl<T, I: SparseIndex> SparseMatrix<T, I> {
...
pub struct CooMatrix<T, I: SparseIndex = u32> {
    pub nrows: usize,
    pub ncols: usize,
    pub rows: Vec<I>,
    pub cols: Vec<I>,
...
impl<T: Scalar, I: SparseIndex> CooMatrix<T, I> {
    ...
    pub fn push(&mut self, row: usize, col: usize, value: T) {
        self.rows.push(I::from_usize(row));
        self.cols.push(I::from_usize(col));
        self.values.push(value);
    }
    #[must_use]
    pub fn finish_csr(self) -> SparseMatrix<T, I> {
        self.compress(Orientation::Csr)
    }
    #[must_use]
    pub fn compress(self, orientation: Orientation) -> SparseMatrix<T, I> {
        ...
        let key = |k: usize| match orientation {
            Orientation::Csr => (self.rows[k].to_usize(), self.cols[k].to_usize()),
            Orientation::Csc => (self.cols[k].to_usize(), self.rows[k].to_usize()),
        };
        order.sort_by_key(|&k| key(k));
        let mut offsets = Vec::with_capacity(major + 1);
        ...
        let mut p = 0usize;
        while p < order.len() {
            let (maj, min) = key(order[p]);
            while cur < maj {
                offsets.push(I::from_usize(indices.len()));
                cur += 1;
            }
            let mut value = self.values[order[p]];
            let mut q = p + 1;
            while q < order.len() && key(order[q]) == (maj, min) {
                value += self.values[order[q]];
                q += 1;
            }
            indices.push(I::from_usize(min));
            values.push(value);
            p = q;
        }
        while cur < major {
...
pub enum NumericError {
    ...
    #[error("operation unsupported: {what}")]
    Unsupported { what: String },
    #[error("transpose action is unsupported")]
...
pub trait Jacobian<T> {
    ...
    fn assemble_into(
        ...
        ctx: &Ctx,
        x: &[T],
        out: &mut SparseMatrix<T>,
    ) -> Result<(), NumericError>;
    fn jvp(&self, ctx: &Ctx, x: &[T], v: &[T], out: &mut [T]) -> Result<(), NumericError> {
        let _ = (ctx, x, v, out);
        Err(NumericError::Unsupported {
            what: "matrix-free jvp".into(),
        })
    }
}
...
impl<S: Scalar> IntegratorCoeffs<S> {
    ...
    #[must_use]
    pub fn generalized_alpha(mass: S, damp: S, stiff: S) -> Self {
        Self { mass, damp, stiff }
    }
}
...
pub trait DaeResidual<S: Scalar>: Jacobian<S> {
    ...
    fn iteration_matrix(
        ...
        Err(NumericError::Unsupported {
            what: "assembled DAE iteration_matrix".into(),
        })
    }
...
mod tests {
    use super::*;
    #[test]
    fn coo_compression_sums_duplicates_deterministically() {
        let mut c = CooMatrix::<f64>::new(2, 2);
        c.push(0, 1, 2.0);
        c.push(0, 1, 3.0);
        let m = c.finish_csr();
        assert_eq!(m.values, vec![5.0]);
        assert_eq!(m.pattern.offsets, vec![0, 1, 1]);
    }
}
